...
if (FileUpload.isMultipartContent(request)) {
	...
	//3. MultipartRequest(request, 위치, 사이즈, encode, 이름정책) 객체와 같은 기능
	...
	for (int i = 0; i < fileItemList.size(); i++) {
		...
		//4.2 폼파라미터 보내온 <input type="file">이 아닌것만 찾음
		if (fileItem.isFormField()) {// 파일 이외의 파라미터
			...
			//메모리에 저장되어있는 파일, 디스크에 저장되어 있는 파일
	...
	if (fileItem.getSize() > 0) {
		//D:\\temp\\orange.png \\없으면 위치는 -1
		int idx = fileItem.getName().lastIndexOf("\\");
		if (idx == -1) {
			//D:/temp/orange.png \\없으면 위치는 -1
			idx = fileItem.getName().lastIndexOf("/");
		}
		String fileName = fileItem.getName().substring(idx + 1);
		try {
			//filepath String filepath = application.getRealPath("upload");
			//new upload/orange.png
			File uploadedFile = new File(filepath, fileName);
			//d://temp/orange.png => /jspStudy/upload/orange.png 임시파일을 직접 옮김
